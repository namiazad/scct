...
class ScctTransformComponent(val global: Global, val opts: ScctInstrumentPluginOptions) extends PluginComponent with TypingTransformers with Transform {
  ...
  class Instrumenter(unit: CompilationUnit) extends TypingTransformer(unit) {
    ...
    private def hasSkipAnnotation(t: Tree) = t.hasSymbol && t.symbol.hasAnnotation(rootMirror.getClassByName(global.stringToTypeName("com.sqality.scct.uncovered")))
    ...
    private def rawCoverageCall(id: Int) = {
      val fun = Select(Select(Select(Select(Ident("com"), newTermName("sqality")), newTermName("scct")), newTermName("Coverage")), newTermName("invoked"))
